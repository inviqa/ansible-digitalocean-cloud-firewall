...
This repository includes a `Jenkinsfile` for running the same
Workspace-backed validation, live test, and release preflight flow used during
local maintenance.
...
- the live DigitalOcean Cloud Firewall test harness through
...
The live test stage provisions real DigitalOcean resources, so it incurs
provider cost while it runs.

The Jenkins pipeline mirrors the safe local live-test path. The stage runs
`ws test-live full-cycle`, and the pipeline post-action runs
`ws test-live cleanup` again as an idempotent safety net before destroying the
Workspace environment. The diagrams split the pipeline into validation,
...
flowchart LR
  ...
  lint --> syntax_check["ws ansible syntax"]
  syntax_check --> preflight["Release preflight"]
```

The live-test gate either runs the DigitalOcean full-cycle test or hands the
build directly to release publication gates.

```mermaid
flowchart LR
  preflight["Release preflight"] --> live_gate{"RUN_LIVE_TESTS?"}
  live_gate -->|Yes| live["Live test"]
  live_gate -->|No| publish["Publication gates"]
  live --> publish
```
...
flowchart LR
  ...
  github_gate -->|Yes| github["GitHub release"]
  github_gate -->|No| galaxy_gate{"Galaxy flag?"}
  github --> galaxy_gate
  galaxy_gate -->|Yes| galaxy["Galaxy import"]
...
flowchart LR
  ...
  failure -->|No| live_cleanup{"Live tests ran?"}
  slack --> live_cleanup
  live_cleanup -->|Yes| cleanup["Cleanup"]
  live_cleanup -->|No| destroy["ws destroy"]
  cleanup --> destroy
...
| --- | --- | --- |
| `inviqa-ansible-roles-releases` | Secret text | GitHub API token used to create the release in `inviqa/ansible-digitalocean-cloud-firewall`. |
...
example, they can disable live tests, publish a specific `RELEASE_VERSION`, or
reimport Galaxy while skipping GitHub release creation.
...
by the fixed credential IDs listed above. Change those IDs only when the
Jenkins job wiring changes; use build parameters for per-run operator choices.
...
- GitHub repository: `inviqa/ansible-digitalocean-cloud-firewall`
...
- Galaxy GitHub owner/repository:
  `inviqa/ansible-digitalocean-cloud-firewall`
- Galaxy branch: `main`
- Galaxy role name: `digitalocean_cloud_firewall`
- Galaxy version check: pinned install of
  `inviqa.digitalocean_cloud_firewall,<release-version>`
