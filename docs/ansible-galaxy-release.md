...
`inviqa.digitalocean_cloud_firewall` Ansible role to Ansible Galaxy after a
...
Jenkins, or live-test workflows documented in [Testing](testing.md) and
...
`ansible-galaxy role info inviqa.digitalocean_cloud_firewall` after a
successful import.
...
| --- | --- | --- |
| `inviqa-ansible-roles-releases` | Secret text | Creates the GitHub release in `inviqa/ansible-digitalocean-cloud-firewall`. |
...
version="0.1.0"
tmp_dir="$(mktemp -d)"
ansible-galaxy role install --roles-path "${tmp_dir}" "inviqa.digitalocean_cloud_firewall,${version}"
